...
var idl_search = require('./src/bacardi/idl_search');
var idl_parser = require('./src/bacardi/idl_parser');
...
var include_bacardi = [path.join(__dirname, 'src', 'bacardi')];

function bacardi() {
	//1. search IDL
	//var idl_list = ["./calculator.idl"];
	var idl_list = idl_search.search();
	//2. parse IDL to AST tree using webidl
	idl_list.forEach(function (idl_file) {
	  idl_file = path.resolve(idl_file);
	  var parsed_data = idl_parser.parse(idl_file);
...
module.exports = {
  ...
  include_bacardi: include_bacardi.map(function(item) {
    return '"' + item + '"';
  }).join(' '),
  bacardi: bacardi,
  gyp: gyp,
